...
title: smileyCTF
date: 2025-02-16 
description : 'smileyCTF'
tags: ['kernel','heap','rop']
author: ['kuvee']
draft: false
---
...
int main () {
    ...
    int fd = open("/dev/blargh", 0x0);
    printf("%d\n", fd);
    int a = ioctl(fd, 0x40086721, 0x18428c3);
    printf("%d\n", a);
...
- option1: nhập 1 `idx` và 1 `size` với điều kiện `size` bé hơn  0xf8 , sau đó malloc và check với địa chỉ được `sbrk(0)` lúc nãy , nếu chunk được malloc trả về bé hơn thì nó sẽ gán `chunk[idx] = 0` , có nghĩa là ta không thao tác được gì với chunk này 
